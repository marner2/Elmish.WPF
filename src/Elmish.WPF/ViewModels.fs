  ...
  member _.Get<'a> ([<CallerMemberName>] ?memberName: string) =
    fun (binding: string -> Binding<'model, 'msg>) ->
      ...
      | None ->
        ...
        failwithf "[%s] Get FAILED: Binding %O could not be constructed" nameChain memberName
      | Some (Error e) ->
        match e with
        ...
        failwithf "[%s] Get FAILED: Binding %O returned an error %O" nameChain memberName e
      | Some (Ok r) -> r |> unbox<'a>
